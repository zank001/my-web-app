...
flowchart TB
    ...
    subgraph NORM["Normalization"]
        N1["brand → generic (drug_details)<br/>→ generic_name + ATC code + ATC class"]
    end
...
|---|---|---|
| `id` | UUIDv7 | PK |
| `rule_type` | enum `cds_rule_type` | `DDI` \| `DRUG_PREGNANCY` \| `DRUG_LACTATION` \| `DUPLICATE_THERAPY` — กฎยา-โรคแยกเก็บในตาราง `drug_disease_rules` (โครง ICD-10 จับกับ `patient_conditions` — DDL ใน schema doc) |
| `left_kind` / `right_kind` | enum `cds_code_kind` | `GENERIC` \| `ATC_CLASS` — ระบุว่าฝั่งซ้าย/ขวาเป็น generic หรือ class (`right_kind` เป็น NULL สำหรับกฎเดี่ยว pregnancy/lactation) |
| `left_code` / `right_code` | text | ค่า `generic_name` (normalize lower — canonical ตาม `drug_details.generic_name`) หรือ ATC code (เช่น `C09` = ACE inhibitors ระดับ class) |
| `condition_code` | text null | บริบทเพิ่มเติมของกฎเดี่ยว เช่น `TRIMESTER_1` สำหรับกฎ pregnancy |
...
| `evidence_level` | enum `cds_evidence_level` | `A` \| `B` \| `C` (คุณภาพหลักฐาน) |
| `source_ref` | text | อ้างอิงแหล่ง (ชื่อฐานข้อมูล + id ในฐาน หรือ citation) — **บังคับกรอก** |
| `ruleset_version` | int | เวอร์ชันชุดกฎที่กฎนี้อยู่ (FK → `cds_rulesets.version` ดู §2.7; NULL เฉพาะกฎที่ร้านเพิ่มเอง) |
...
flowchart LR
    ...
    P --> DD["drug_details<br/>(generic_name, atc_code, strength, form,<br/>is_high_alert, preg_category)"]
    DD --> KEY["canonical key:<br/>{generic_name, atc_code,<br/>atc_class (ตัดจาก atc_code)}"]
    KEY --> SET["สร้าง set ของ generic_name + ATC class<br/>ของทั้งตะกร้า + ยา 90 วัน"]
```

- ยาแต่ละ `product` ผูกกับ `drug_details` ที่มี `generic_name` (normalize lower — ค่า canonical
  ภายในระบบ ควร map กับมาตรฐานเช่น TMT/INN) และ `atc_code` (WHO ATC) — จาก `atc_code` ตัดเอาระดับ
  class ได้หลายชั้น (เช่น `C09AA05` → `C09AA`, `C09A`, `C09`) เพื่อจับกฎระดับ class
- ยาที่ไม่มี `generic_name`/`atc_code` (สินค้าไม่ใช่ยา หรือยังไม่ map) → **ข้ามการตรวจแต่ติดธง `unmapped`
  ใน response** ให้เภสัชกรรู้ว่ารายการนี้ยังไม่ได้ตรวจ (เหมือนเช็คไม่ได้ ไม่ใช่ผ่าน) — สำคัญมาก อย่านับเป็น "ปลอดภัย"
- ยาสูตรผสม (combination) มีได้หลาย component ต่อ 1 product → ขยายเป็นหลาย `generic_name` ก่อนตรวจ
...
```
รวบรวม items = ตะกร้า ∪ ยา 90 วัน (แต่ละตัวมี generic_name + ชุด ATC class หลายชั้น)

# 1) DDI — pairwise ระดับ generic และ class
codes = union ของ generic_name + ATC class ทุกชั้น ของทุก item   # set
...
- **Exact match**: `patient_allergies.generic_name` ตรงกับ `generic_name` ของยาที่จ่าย → alert
...
- **Cross-reactivity group** ผ่านตาราง `allergy_cross_groups` — จับกลุ่มที่มีปฏิกิริยาข้ามกัน เช่น
  ...
  - โครงสร้าง (ตาม schema doc): `allergy_cross_groups(code, name, member_generics[],
    cross_reactive_group_codes[], cross_reactivity_note)` — 1 แถวต่อกลุ่ม เก็บสมาชิกเป็น array;
    ผู้ป่วยแพ้ผ่าน `patient_allergies.allergy_cross_group_id` (แพ้ทั้งกลุ่ม) หรือแพ้ generic เดี่ยวที่เป็น
    สมาชิกกลุ่ม → เตือนเมื่อจ่ายสมาชิกอื่นในกลุ่มเดียวกัน/กลุ่มที่ cross-react ระดับความเสี่ยงระบุใน
    `cross_reactivity_note` ของกลุ่ม

**Drug-disease check** (ตาราง `drug_disease_rules` — จับ `atc_prefix`/`generic_name` กับ
`patient_conditions.icd10_code` ผ่าน `icd10_prefix`) เช่น
...
**Drug-pregnancy / lactation** (`rule_type = DRUG_PREGNANCY` / `DRUG_LACTATION`)
- ใช้ flag `patients.is_pregnant` / `patients.is_lactating` + `drug_details.preg_category`
...
`cds_alerts` เป็นตาราง **append-only** (trigger ห้าม UPDATE/DELETE — ดู schema doc): 1 alert ที่ผู้ใช้
ตัดสินใจแล้ว = insert 1 แถว **ครั้งเดียวจบ** ไม่มีการอัปเดตสถานะภายหลัง (โครงสร้างสุดท้ายใน schema doc):
...
| `rule_ref_type`, `rule_ref_id`, `ruleset_version` | กฎที่ยิง (`'ddi_rules'` \| `'drug_disease_rules'` \| `'patient_allergies'` \| ...) + เวอร์ชันชุดกฎ (reproducible; NULL เมื่อไม่ได้มาจากกฎกลาง เช่น allergy จากข้อมูลผู้ป่วย/กฎร้าน) |
| `alert_type`, `severity` | ชนิด + ระดับ |
| `left_code`, `right_code` | คู่ยา/ยา-ภาวะที่ชน (snapshot) |
| `message` | ข้อความ alert ที่แสดงจริง (snapshot) |
| `outcome` | `ACCEPTED` (ทำตามคำแนะนำ/รับทราบ) \| `OVERRIDDEN` (ยืนยันจ่ายต่อ) \| `BLOCKED` (ระบบบล็อกเด็ดขาด/ผู้ใช้ถอนรายการ) |
| `override_reason` (nullable) | เหตุผลบังคับเมื่อ `outcome = OVERRIDDEN` (CHECK ระดับ DB) |
| `decided_by`, `triggered_at` | ใครตัดสินใจ (ต้อง PHARMACIST สำหรับ override CONTRAINDICATED/MAJOR), เมื่อไร |

จังหวะการเขียน: `POST /cds/check` เป็น **read-only** (ไม่เขียนอะไร) — แถว `cds_alerts` ถูก insert
เมื่อผู้ใช้ตัดสินใจต่อ alert (ผ่าน `POST /cds/alerts/{id}/act` — OVERRIDDEN/BLOCKED) หรือเมื่อปิดบิล
(alert ระดับ MODERATE/MINOR ที่รับทราบ → ACCEPTED ใน transaction เดียวกับการขาย) alert ของตะกร้า
ที่ถูกยกเลิกทิ้งไม่บันทึก
...
- **Pipeline**: ภาพใบสั่งยา → OCR → **NER** ดึง (drug name, strength, form, dose, frequency, duration)
  → **normalize เข้า generic_name/product** (fuzzy match กับ catalog) → **เภสัชกรยืนยันทุกครั้ง (human-in-the-loop)**
...
แบบ opt-in/de-identified ตาม PDPA §6 ของ `01-architecture.md` — DDL อยู่ใน schema doc §3.6):
...
- ทุกการจ่าย antibiotic บันทึกให้คำนวณ metric ได้: `generic_name`+ATC (J01 = antibacterials for
  systemic use), จำนวนที่จ่าย, ขนาด/วัน, ระยะเวลา, syndrome (เก็บที่ `dispense_records.syndrome_code`
  เมื่อเลือกผ่าน §5.2), มี/ไม่มีใบสั่งแพทย์ — ค่า DDD อ้างอิงต่อยาอยู่ที่ `drug_details.ddd_value/ddd_unit`
...
Endpoint หลัก: **`POST /api/v1/cds/check`** — ตรวจทุกมิติในครั้งเดียว, **read-only/idempotent**
(ไม่เขียน state ใด ๆ — `cds_alerts` ถูก insert ตอนผู้ใช้ตัดสินใจ/ปิดบิล ดู §3.3; ระหว่างนั้น server เก็บ
pending alert ไว้ใน Redis คีย์ `alert_id` TTL สั้น ๆ เพื่อรอการ act), เป้า < 100ms
...
- server **insert** แถว `cds_alerts` (outcome = `OVERRIDDEN`; ACKNOWLEDGE ของ alert ระดับ soft →
  `ACCEPTED`) และเขียน `audit_logs` — append-only ไม่มีการ UPDATE (§3.3); ปลดล็อกปุ่มชำระเงินเมื่อ
  blocking alert ทั้งหมดถูก override/แก้ไข (เอายาออก) แล้ว
...
- **alert_type enum (`cds_alert_type` — ตรงกับ schema doc)**: `DDI`, `ALLERGY`, `DRUG_DISEASE`,
  `DRUG_PREGNANCY`, `DRUG_LACTATION`, `DUPLICATE_THERAPY`, `HIGH_ALERT`, `ANTIBIOGRAM`
- กฎเก็บระดับ **generic / ATC class** ใน `ddi_rules` (กฎยา-โรคใน `drug_disease_rules`);
  normalize brand→generic ผ่าน `drug_details`; ยา unmapped = "ตรวจไม่ได้" ไม่ใช่ "ปลอดภัย"
- ตรวจรวมยาในตะกร้า + **ยา 90 วันล่าสุดจาก `dispense_records`** + current meds ที่กรอกมือ
- ทุกการตัดสินใจต่อ alert insert ลง `cds_alerts` (append-only, + `audit_logs`) พร้อม
  `rule_ref_type/rule_ref_id`, `ruleset_version`, `outcome` (ACCEPTED/OVERRIDDEN/BLOCKED)
