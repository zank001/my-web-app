...
suppliers, lots, stock_levels, inventory_movements, reorder_rules, purchase_orders,
purchase_order_items, goods_receipts, goods_receipt_items, receipt_counters,
receipt_number_ranges, sales, sale_items, sale_payments, notifications, audit_logs`

### 3.2 Deliverables + Effort

| # | งาน | รายละเอียด | สัปดาห์ |
|---|---|---|---|
| 1 | Backend skeleton | โครง Modular Monolith ตาม `01-architecture.md` §2, Pydantic Settings, Alembic, docker-compose 6 containers ตาม `01-architecture.md` §4.1 (caddy/api/worker/beat/postgres/redis), CI GitHub Actions | 2.5 |
...
| 11 | Deploy + pilot | VPS จริง, HTTPS, backup อัตโนมัติ (pgBackRest + WAL archiving ตาม `01-architecture.md` §4.5), ใช้ขายจริงคู่ขนาน 2 สัปดาห์แล้วตัดเข้าระบบเดียว | 1.5 |
| | **รวม** | | **16–20** |
...
ตารางใหม่: `patients, patient_allergies, patient_conditions, cds_rulesets, ddi_rules,
drug_disease_rules, allergy_cross_groups, cds_alerts, controlled_drug_registers,
regulatory_reports, licenses` (+ เติม `drug_details` ให้ครบ: ประเภทยาตามกฎหมาย, ATC, คำเตือน)
...
| 7 | Licenses + GPP alert | ทะเบียนใบอนุญาต (ขย., ใบประกอบวิชาชีพ, GPP) + Celery เตือนล่วงหน้าตามบันได 90/60/30/14/7 วันก่อนครบกำหนดต่ออายุ (`05-gpp-compliance.md` §5.1) | 1.0 |
...
ตารางใหม่: `loyalty_tiers, point_transactions, refill_reminders, prescriptions,
prescription_items, dispense_records` (+ เปิดใช้ `branches` มากกว่า 1 แถวจริง ๆ)
