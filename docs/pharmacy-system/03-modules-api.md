...
เอกสารนี้ลงรายละเอียด **7 โมดูล** ของระบบจัดการร้านยา: user story/flow หน้างานจริง,
...
- ⚠️ **ต้องเพิ่มในการปรับ schema ครั้งถัดไป**: คอลัมน์ `sales.idempotency_key varchar(26)` +
  `UNIQUE (branch_id, idempotency_key)` (สถาปัตยกรรมระบุพฤติกรรมนี้ไว้แล้วแต่ DDL ใน 02 ยังไม่มีคอลัมน์)
...

{
  ...
  "items": [
    { "product_id": "018f9a2c-7e10-7c02-8000-aaaa0002", "qty": "20.000",
      "unit_price": "3.50", "item_discount": "0.00",
  ...
  "bill_discount": "5.00",
  "payments": [ { "method": "PROMPTPAY", "amount": "75.00", "ref_no": "PP20260709xxxx" } ],
...

{
  ...
  "status": "COMPLETED",
  "subtotal": "80.00", "bill_discount": "5.00", "vat_amount": "0.00", "total": "75.00",
  "points_earned": 7,
...
- **แจ้งเตือนใกล้หมดอายุ**: job รายวันตาม query §5.3 ของ schema doc → notification `EXPIRY_WARNING`
  ที่ 180/90/30 วัน → หน้า expiry dashboard (§3.4)
...
POST /api/v1/cds/check
{
  ...
  "extra_current_medications": [ { "generic_id": "warfarin" } ]
}
...
| `lot.expiring` | inventory (Celery scan) | lot_id, exp_date, days_left, value | notifications: `EXPIRY_WARNING` (บันได 180/90/30 วัน) | async |
...
- สิ่งที่ **ต้องเพิ่มในการปรับ schema ครั้งถัดไป** (อ้างแล้วในเอกสารนี้):
  1. `sales.idempotency_key` + unique index (§1.4)
  2. merge addendum จาก `05-gpp-compliance.md` §8: `pharmacist_duty_sessions`, `temperature_logs`,
     `patient_consents`, คอลัมน์ `drug_details.requires_ky11/requires_ky13/max_qty_per_sale/max_qty_note`,
