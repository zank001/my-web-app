...
| **พ.ร.บ.วัตถุที่ออกฤทธิ์ต่อจิตและประสาท พ.ศ. 2559** | วัตถุออกฤทธิ์ประเภท 3/4 ขายได้เฉพาะผู้รับอนุญาตขายวัตถุออกฤทธิ์ ต้องทำบัญชีรับ-จ่าย + รายงานต่อ อย. ตามรอบ | `legal_category = 'PSYCHOTROPIC_3_4'`, `controlled_drug_registers (register_type='PSYCHOTROPIC')`, license type `PSYCHOTROPIC_LICENSE` |
| **ประมวลกฎหมายยาเสพติด พ.ศ. 2564** (แทน พ.ร.บ.ยาเสพติดให้โทษ พ.ศ. 2522) | ยาเสพติดให้โทษประเภท 3 (ตำรับที่มียาเสพติดประเภท 2 ผสม เช่น ตำรับโคเดอีน) ต้องมีใบอนุญาตจำหน่าย, บัญชีรับ-จ่าย, รายงานต่อ อย. | `legal_category = 'NARCOTIC_3'`, `controlled_drug_registers (register_type='NARCOTIC')`, license type `NARCOTIC_LICENSE` |
...
| `DANGEROUS` | ยาอันตราย | **เภสัชกรเท่านั้น** (ผู้มีหน้าที่ปฏิบัติการ) | ไม่ (แต่เภสัชกรต้องซักถาม-คัดกรอง) | ไม่ | ข.ย.9 + ข.ย.11 เฉพาะรายการที่ อย. กำหนด (เช่นกลุ่ม tramadol, dextromethorphan ⚠️ ดูประกาศรายตัวยา) |
...
ยาบางกลุ่มมีประกาศจำกัดปริมาณการขายต่อคนต่อครั้ง (เช่น tramadol, ยาแก้แพ้/แก้ไอบางตำรับ,
...
| **ข.ย.13** | รายงานการขายยาตามที่เลขาธิการ อย. กำหนด | `KY13_ANNUAL_SALE` | aggregate จาก `sale_items` (filter flag `requires_ky13`) | ส่ง อย./สสจ. ตามรอบที่ประกาศรายตัวยากำหนด ⚠️ (บางรายการรายปี บางรายการถี่กว่า) |
...
### 3.3 วัตถุออกฤทธิ์ประเภท 3/4 (พ.ร.บ.วัตถุที่ออกฤทธิ์ต่อจิตและประสาท พ.ศ. 2559)
...
2. **รายงานประจำเดือน + รายงานประจำปี ต่อ อย.** → `regulatory_reports` ด้วย `report_type = 'PSY_MONTHLY'` / `'PSY_ANNUAL'`
   ...
   ⚠️ **เลขแบบฟอร์ม** (แบบบัญชี/แบบรายงาน เช่น ชุด "บ.จ." และแบบรายงานตามกฎกระทรวงภายใต้ พ.ร.บ. 2559)
...
เอกสารนี้พบ gap 4 จุดที่ schema ปัจจุบันยังไม่มี — เสนอ DDL ให้ schema doc รับไปพิจารณา merge
...
3. เลขแบบบัญชี/รายงานวัตถุออกฤทธิ์ 3/4 (ชุด "บ.จ." ฯลฯ) + รอบส่ง + format ระบบ e-submission วัตถุเสพติด — จาก narcotic.fda.moph.go.th
...
5. รายการยาที่ต้องทำ ข.ย.11 / รายงาน ข.ย.13 + ปริมาณจำกัดต่อครั้ง (เช่น tramadol) — ประกาศ อย. รายตัวยาฉบับล่าสุด → เก็บเป็น seed data ของ flag `requires_ky11/ky13` + `max_qty_per_sale`
