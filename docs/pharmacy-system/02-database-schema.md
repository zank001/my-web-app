...
`cds_rulesets`, `ddi_rules`, `drug_disease_rules`, `allergy_cross_groups`, `antibiograms`
(ตารางกฎที่ร้านเพิ่ม rule เองได้ใช้ `tenant_id` nullable + policy แยก — ดู DDL)
...
CREATE TABLE drug_details (                      -- 1:1 กับ products เฉพาะ is_drug = true
  ...
  preg_category  varchar(20),                    -- หมวดตั้งครรภ์ (A/B/C/D/X หรือ narrative code) — CDS ตัดสินด้วยกฎ generic-level เป็นหลัก ดู 04 §2.5
  ddd_value      NUMERIC(10,4),                  -- WHO DDD ต่อหน่วยพื้นฐาน — ใช้คำนวณ antibiotic stewardship metric (04 §5.3)
  ddd_unit       varchar(10),                    -- หน่วยของ DDD เช่น 'g', 'mg'
...
CREATE TABLE receipt_number_ranges (             -- จองช่วงเลขใบเสร็จล่วงหน้าให้ POS terminal ใช้ตอน offline
  id           uuid PRIMARY KEY DEFAULT uuid_generate_v7(),
  tenant_id    uuid NOT NULL REFERENCES tenants(id),
  branch_id    uuid NOT NULL REFERENCES branches(id),
  terminal_id  text NOT NULL,                    -- เครื่อง POS ที่ถือช่วงเลขนี้
  period       char(6) NOT NULL,                 -- งวดเดียวกับ receipt_counters
  range_start  int NOT NULL,
  range_end    int NOT NULL,
  issued_upto  int NOT NULL DEFAULT 0,           -- เลขล่าสุดที่ terminal ใช้แล้ว (อัปเดตตอน sync)
  allocated_at timestamptz NOT NULL DEFAULT now(),
  updated_at   timestamptz NOT NULL DEFAULT now(),
  CHECK (range_start <= range_end),
  -- กันช่วงเลขทับซ้อนกันภายในสาขา+งวดเดียวกัน (ต้องมี extension btree_gist)
  EXCLUDE USING gist (
    branch_id WITH =, period WITH =,
...
CREATE TABLE sales (
  ...
  idempotency_key varchar(26),                   -- ULID จาก POS terminal — กัน double-submit จาก offline queue
...
CREATE TABLE patients (
  ...
  is_pregnant      boolean NOT NULL DEFAULT false, -- ใช้กับ CDS DRUG_PREGNANCY — เภสัชกรถามยืนยันทุกครั้งที่จ่ายยากลุ่มเสี่ยง
  is_lactating     boolean NOT NULL DEFAULT false, -- ใช้กับ CDS DRUG_LACTATION
...
CREATE TABLE dispense_records (                  -- บันทึกการส่งมอบยา: ใคร จ่ายอะไร ให้ใคร เมื่อไหร่
  ...
  syndrome_code   text,                          -- กลุ่มอาการที่เลือกตอนจ่าย antibiotic (stewardship — 04 §5.2/5.3)
...
กฎกลางทุกตัวผูกกับ **ชุดกฎแบบ versioned immutable** ผ่าน `cds_rulesets` (กระบวนการ review →
golden test → publish อยู่ใน `04-ddi-engine.md` §2.7) — กฎที่ร้านเพิ่มเอง (`tenant_id` ไม่ NULL)
ไม่เข้ารอบ version (`ruleset_version` เป็น NULL มีผลทันที) และ `cds_alerts` ที่เกิดจากกฎร้านจะอ้าง
`rule_ref_id` ตรง ๆ แทน

`ddi_rules` ใช้โมเดล **generic rule** (ตาม `04-ddi-engine.md` §2.1): 1 แถว = กฎ 1 ข้อ ชนิดตาม
`rule_type` — กฎแบบคู่ (`DDI`, `DUPLICATE_THERAPY`) มีทั้ง `left_*` และ `right_*`, กฎแบบเดี่ยว
(`DRUG_PREGNANCY`, `DRUG_LACTATION`) มีเฉพาะ `left_*` ส่วนกฎยา-โรคแยกเก็บใน `drug_disease_rules`
(โครงตาม ICD-10 จับกับ `patient_conditions`)

```sql
CREATE TABLE cds_rulesets (                      -- ชุดกฎ CDS แบบ versioned immutable (ความรู้กลาง — ไม่มี tenant_id)
  id            uuid PRIMARY KEY DEFAULT uuid_generate_v7(),
  version       int NOT NULL UNIQUE,             -- รันเพิ่มทีละ 1 เมื่อ publish
  status        cds_ruleset_status NOT NULL DEFAULT 'DRAFT',
  changelog     text,
  reviewed_by   text,                            -- ชื่อ + เลขใบประกอบวิชาชีพเภสัชกรผู้ review (อาจอยู่นอกระบบร้าน)
  golden_test_passed boolean,                    -- ผล golden regression suite ก่อน publish (ดู 04 §2.7)
...
CREATE TABLE ddi_rules (                         -- กฎ CDS แบบคู่/เดี่ยว: DDI, duplicate therapy, pregnancy, lactation
  id           uuid PRIMARY KEY DEFAULT uuid_generate_v7(),
  tenant_id    uuid REFERENCES tenants(id),      -- NULL = rule กลางของระบบ
  rule_type    cds_rule_type NOT NULL,
  left_kind    cds_code_kind NOT NULL,           -- GENERIC | ATC_CLASS
  left_code    text NOT NULL,                    -- generic_name (lower) หรือ ATC prefix เช่น 'M01A'
  right_kind   cds_code_kind,                    -- NULL สำหรับกฎเดี่ยว (pregnancy/lactation)
  right_code   text,                             -- กฎแบบคู่เก็บเรียง left_code <= right_code (กันกฎซ้ำ 2 ทิศ)
  condition_code text,                           -- บริบทเพิ่มเติม เช่น 'TRIMESTER_1' สำหรับกฎ pregnancy
  ...
  evidence_level cds_evidence_level,             -- คุณภาพหลักฐาน A/B/C
  source_ref   text NOT NULL,                    -- แหล่งอ้างอิง (บังคับ) เช่น 'Stockley 2024', 'Micromedex'
  ruleset_version int REFERENCES cds_rulesets(version), -- NULL เฉพาะกฎที่ร้านเพิ่มเอง
  is_active    boolean NOT NULL DEFAULT true,
  created_at   timestamptz NOT NULL DEFAULT now(),
  updated_at   timestamptz NOT NULL DEFAULT now(),
  CHECK ((rule_type IN ('DDI', 'DUPLICATE_THERAPY')) = (right_code IS NOT NULL)),
  CHECK (right_code IS NULL OR left_code <= right_code),
  CHECK (tenant_id IS NOT NULL OR ruleset_version IS NOT NULL), -- กฎกลางต้องมี version
  UNIQUE NULLS NOT DISTINCT
    (tenant_id, rule_type, left_kind, left_code, right_kind, right_code, ruleset_version)
);
...
CREATE TABLE allergy_cross_groups (              -- กลุ่ม cross-reactivity เช่น penicillins ↔ cephalosporins
  ...
  ruleset_version int REFERENCES cds_rulesets(version), -- NULL เฉพาะกลุ่มที่ร้านเพิ่มเอง
  is_active     boolean NOT NULL DEFAULT true,
  created_at    timestamptz NOT NULL DEFAULT now(),
  updated_at    timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE drug_disease_rules (                -- ข้อห้าม/ข้อควรระวัง ยา-โรค
  ...
  evidence_level cds_evidence_level,
...
CREATE TABLE cds_alerts (                        -- append-only: log ทุก alert + ผลการตัดสินใจ (GPP audit)
  ...
  rule_ref_id  uuid,
  ruleset_version int,                           -- เวอร์ชันชุดกฎที่ใช้ตอนตรวจ (NULL เมื่อ alert ไม่ได้มาจากกฎกลาง เช่น ALLERGY/HIGH_ALERT/กฎร้าน)
  left_code    text,                             -- code ฝั่งซ้ายที่ชน (generic/ATC/allergy group) — snapshot เพื่อตรวจย้อน
  right_code   text,                             -- code ฝั่งขวา (NULL สำหรับ alert แบบเดี่ยว)
...
```sql
CREATE TABLE antibiograms (                      -- ความไวเชื้อระดับพื้นที่/เครือข่าย (ส่วนขยาย AMR — Phase 4, ดู 04 §5)
  id              uuid PRIMARY KEY DEFAULT uuid_generate_v7(),
  region_code     text NOT NULL,                 -- ขอบเขตพื้นที่ เช่น รหัสจังหวัด/เขตสุขภาพ
  facility_scope  text,                          -- แหล่งข้อมูล เช่น 'รพ.แม่ข่าย', 'เครือข่ายร้านยา' (de-identified, opt-in)
  year            int NOT NULL,                  -- ปีของ cumulative antibiogram (อัปเดตรายปี)
  organism        text NOT NULL,                 -- เช่น 'Escherichia coli'
  specimen_source text NOT NULL,                 -- 'urine' | 'throat' | 'wound' | 'blood' — ความไวต่างกันตาม source
  antibiotic      text NOT NULL,                 -- generic_name (lower) — map ATC ผ่าน drug_details
  n_isolates      int NOT NULL,                  -- < เกณฑ์ขั้นต่ำ (~30, CLSI M39 ⚠️ ตรวจฉบับล่าสุด) → UI แสดง "ข้อมูลไม่พอ"
  pct_susceptible NUMERIC(5,2) NOT NULL CHECK (pct_susceptible BETWEEN 0 AND 100),
  source_note     text,
  created_at      timestamptz NOT NULL DEFAULT now(),
  updated_at      timestamptz NOT NULL DEFAULT now(),
  UNIQUE (region_code, facility_scope, year, organism, specimen_source, antibiotic)
);
```
...
CREATE INDEX idx_ddi_left ON ddi_rules (rule_type, left_code) WHERE is_active;
CREATE INDEX idx_ddi_right ON ddi_rules (rule_type, right_code) WHERE is_active;
...
```sql
-- รวบ generic ของยาในตะกร้า + ยาที่ผู้ป่วยได้รับใน 90 วันล่าสุด (default ตาม 04 §3.2)
-- แล้วจับคู่กับ ddi_rules (โมเดล generic rule — ตัวอย่างนี้จับเฉพาะระดับ GENERIC;
-- ระดับ ATC class ทำที่ engine ซึ่งขยาย ATC ทุกชั้นก่อน lookup — ดู 04 §2.3)
WITH active_drugs AS (
  ...
  WHERE dr.patient_id = :patient_id AND dr.dispensed_at >= now() - INTERVAL '90 days'
)
SELECT r.* FROM ddi_rules r
JOIN active_drugs a ON a.g = r.left_code
JOIN active_drugs b ON b.g = r.right_code
WHERE r.rule_type = 'DDI' AND r.is_active
  AND (r.tenant_id IS NULL OR r.tenant_id = current_setting('app.tenant_id')::uuid)
ORDER BY r.severity;
```
...
- **pgBackRest** (full รายสัปดาห์ + differential รายวัน) + WAL archiving ต่อเนื่อง → PITR
  ตาม `01-architecture.md` §4.5 — RPO ≤ 5 นาที (`pg_dump` ใช้ได้เป็น logical export เสริม
  เช่น export ราย tenant — ไม่ใช่กลไก PITR)
...
## ภาคผนวก: สรุปรายชื่อตารางทั้งหมด (40 ตาราง)
...
| POS | `receipt_counters`, `receipt_number_ranges`, `sales`, `sale_items`, `sale_payments` |
| Patient/Clinical | `patients`, `patient_allergies`, `patient_conditions`, `prescriptions`, `prescription_items`, `dispense_records` |
| CDS | `cds_rulesets`, `ddi_rules`, `allergy_cross_groups`, `drug_disease_rules`, `cds_alerts`, `antibiograms` |
