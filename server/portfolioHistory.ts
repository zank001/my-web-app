...
/**
 * MoneyMap — logic กลางสำหรับคำนวณมูลค่าพอร์ตย้อนหลัง (ใช้ร่วมกันระหว่าง
 * Express API `server/moneymap.ts` และ build script `scripts/genHistory.ts`
 * ที่สร้างไฟล์ static ให้ GitHub Pages)
 *
...
export interface PortfolioHistory {
  ...
  usdThb: number
  fxLive: boolean
  /** ISO timestamp ของตอนที่คำนวณ — ใช้บอกผู้ใช้ว่าข้อมูล static อัปเดตเมื่อไร */
  generatedAt: string
  points: HistoryPoint[]
  assets: AssetSummary[]
}
...
export const DEFAULT_STATE: MoneymapState = {
...
export const ASSET_CLASSES: AssetClass[] = ['thai-equity', 'foreign-equity', 'gold', 'bond', 'cash']

/** ตรวจ/ทำความสะอาดรายการสินทรัพย์หนึ่งรายการ — คืน null ถ้าข้อมูลใช้ไม่ได้ */
export function sanitizeHolding(raw: unknown): Holding | null {
...
export async function readState(): Promise<MoneymapState> {
  let raw: string
  try {
...
export async function writeState(state: MoneymapState): Promise<void> {
 ...
 * vol = แอมพลิจูดของคลื่น sine, trend = แนวโน้มเชิงเส้นตลอดช่วง, period = คาบ (วัน)
 */
...
/** จำนวนเดือนที่รองรับ: 1..24 (ค่าตั้งต้น 6) */
export function clampMonths(input: unknown): number {
  return Math.min(24, Math.max(1, Number(input) || 6))
}

/**
 * คำนวณมูลค่าพอร์ตรวมรายวันตลอดช่วงที่ขอ — หัวใจของทั้ง API และ build script
 * `nowIso` ให้ส่ง timestamp เข้ามาได้ (ไว้ทดสอบ) ปกติใช้เวลาปัจจุบัน
 */
export async function computePortfolioHistory(state: MoneymapState, months: number, nowIso?: string): Promise<PortfolioHistory> {
  ...
  const points: HistoryPoint[] = axis.map((date, i) => {
    let value = 0
